...
export function calendarDayOfYear(month, day) {
    ...
    if (day < 1 || day > len)
        throw new Error(`bad day ${day} for month ${month}`);
    let doy = day;
 ...
 */
export function photoperiodCrossing(lat, hours, direction) {
    for (let j = 2; j <= 365; j++) {
        const prev = daylengthHours(lat, j - 1);
        const cur = daylengthHours(lat, j);
        if (direction === "shortening" && prev >= hours && cur < hours)
            return j;
        if (direction === "lengthening" && prev < hours && cur >= hours)
            return j;
    }
    return null;
}
...
// ---------------------------------------------------------------------------
export function frostKeyOf(ref) {
    return `${ref.thresholdF}/${ref.probabilityPct}`;
}
export function frostDay(site, which, partial) {
    const ref = { ...DEFAULT_FROST_REF, ...partial };
    const key = frostKeyOf(ref);
    const day = site.frost[which][key];
    if (day === undefined) {
        throw new Error(`SiteContext frost table has no ${which} entry for ${key} ` +
...
export function anchorDay(site, anchor) {
    switch (anchor.kind) {
        case "lastFrost":
        ...
        case "photoperiod": {
            ...
            if (day === null) {
                throw new Error(`photoperiod never crosses ${anchor.hours}h (${anchor.direction}) at lat ${site.lat}`);
            }
            return day;
        }
        case "soilTemp":
        case "gddAccum":
            throw new UnsupportedAnchorError(`"${anchor.kind}" is a gate in v1, not a primary anchor — ` +
...
// ---------------------------------------------------------------------------
export function resolveAnchoredEvents(events, site, catalogEntry) {
    const windows = [];
    ...
    for (const ev of events) {
        ...
        if (!src)
            throw new Error(`harvest ${ev.id}: unknown fromEventId "${ev.fromEventId}"`);
        const dth = catalogEntry.daysToMaturity[ev.method];
        if (!dth) {
            throw new Error(`harvest ${ev.id}: catalog entry "${catalogEntry.slug}" has no ` +
...
const CODE_ORDER = ["si", "s", "sg", "t", "tg", "B", "h", "*"];
/**
...
export function frostRegimeApplies(frostFreeDays) {
    return frostFreeDays < FROST_FREE_MAX_DAYS;
}
/** A window covers a slot if it overlaps it by at least one day. */
export function bucketWindows(windows) {
    const covered = SLOTS.map(() => new Set());
    for (const w of windows) {
        if (w.end < w.start)
            throw new Error(`window inverted: [${w.start}, ${w.end}]`);
...
// ---------------------------------------------------------------------------
export function resolveCropTiming(timing, site, catalogEntry) {
...
// ---------------------------------------------------------------------------
export function footprintContains(fp, lat, lng) {
    switch (fp.kind) {
        case "bbox":
        ...
        case "counties":
            throw new Error("counties footprint containment requires a point→FIPS lookup — not implemented in v1");
    }
}
function footprintRank(fp) {
    return fp.kind === "counties" ? 3 : fp.kind === "polygon" ? 2 : 1;
}
function footprintAreaProxy(fp) {
    switch (fp.kind) {
        case "bbox":
...
export function byPrecedence(a, b) {
    ...
    if (area !== 0)
        return area; // smaller area = more specific
...
export function resolveAll(site, input, climate) {
    ...
    const settledByCurated = new Set();
    const out = [];
