...
const PLANTS = [
  ...
  },
  {
    ...
    months: {
      jan: { half1: ["si"], half2: ["si"] },
      feb: { half1: [], half2: ["t"] },
      mar: { half1: ["t", "si"], half2: ["t", "si", "h"] },
      apr: { half1: ["si", "h"], half2: ["si", "h"] },
      may: { half1: ["si", "h"], half2: ["si", "h"] },
      jun: { half1: ["h"], half2: ["t"] },
      jul: { half1: ["t"], half2: ["t", "h"] },
      aug: { half1: ["t", "h"], half2: ["h"] },
      sep: { half1: ["h"], half2: ["h"] },
      oct: { half1: ["h"], half2: ["h"] },
      nov: { half1: [], half2: [] },
      dec: { half1: [], half2: [] }
...
const PLANT_REVIEW_NOTES = {
  ...
  "Celery": "Converted this to a transplant-led schedule. NC State lists outdoor planting primarily as transplant timing; indoor starts were added ahead of those windows because direct sow is not realistic here. Harvest windows are derived arithmetically from these planting windows plus the listed days-to-harvest range (earliest planting + minimum days through latest planting + maximum days), not from a separate source row.",
  ...
  "Leek": "Treated as a transplant-oriented allium with spring S/T timing from the Central NC calendar. Indoor starts were limited to the lead-in for those transplant windows. Harvest windows are derived arithmetically from these planting windows plus the listed days-to-harvest range (earliest planting + minimum days through latest planting + maximum days), not from a separate source row.",
  ...
  "Chives": "Left this mostly intact and slightly broadened spring transplanting. Johnny's and Southern herb guidance support chives as a perennial herb typically started for spring set-out, but they do not give a strong Carrboro half-month calendar, so V2 keeps the conservative late-winter seed-start and early-spring transplant pattern. Harvest windows are derived arithmetically from these planting windows plus the listed days-to-harvest range (earliest planting + minimum days through latest planting + maximum days), not from a separate source row.",
  ...
  "Fennel": "Shifted this to direct-sow timing from the NC State Florence fennel row and the herb-handbook guidance that fennel does not transplant well. Spring sowing now runs March-April, with a second sowing window in July-August. Harvest windows are derived arithmetically from these planting windows plus the listed days-to-harvest range (earliest planting + minimum days through latest planting + maximum days), not from a separate source row.",
  ...
  "Parsnips": "I changed parsnips materially. Instead of starting in February, V2 now treats mid-April to mid-May as the default spring sowing period and keeps the late-August to early-September window for fall overwinter intent, with `*` to flag the crop's slow, moisture-sensitive germination. I also added only a tiny October `sg` shoulder for protected overwinter sowing under the conservative greenhouse rule. Harvest windows are derived arithmetically from these planting windows plus the listed days-to-harvest range (earliest planting + minimum days through latest planting + maximum days), not from a separate source row.",
