...
import type { CropCatalogEntry, OffsetDays, TimingEvent } from "../types.ts";

type DthRange = readonly [number, number];

/** A DTH range is usable when it is a well-formed positive interval. */
function usable(range: DthRange | undefined): DthRange | null {
...
function springTransplantRun(
  events: TimingEvent[],
  indoorOffset: OffsetDays,
  setOutOffset: OffsetDays
): void {
  events.push(
    {
...
function springDirectRun(events: TimingEvent[], sowOffset: OffsetDays): void {
  events.push(
    {
...
function fallDirectRun(events: TimingEvent[], direct: DthRange): void {
 ...
 */
export function computedEvents(entry: CropCatalogEntry): TimingEvent[] | null {
  if (entry.hardiness === "perennial") return null;

  const direct = usable(entry.daysToMaturity.direct);
  const transplant = usable(entry.daysToMaturity.transplant);
  if (!direct && !transplant) return null;
  ...
  switch (entry.hardiness) {
    ...
    case "half-hardy":
      if (transplant) springTransplantRun(events, [-49, -35], [-7, 14]);
      if (direct) {
