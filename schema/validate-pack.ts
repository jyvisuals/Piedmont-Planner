...
const HALF_MONTH_CODES = ["s", "si", "sg", "t", "tg", "B", "h", "*"] as const;
const MONTH_IDS = [
...
const FROST_THRESHOLDS_F = [36, 32, 28, 24, 20, 16] as const;
const FROST_PROBABILITIES_PCT = [90, 80, 70, 60, 50, 40, 30, 20, 10] as const;
const HARVEST_METHODS = ["direct", "transplant"] as const;
const FOOTPRINT_KINDS = ["bbox", "polygon", "counties"] as const;
...
class Errors {
  ...
  at(path: string, reason: string): void {
    this.list.push(`${path === "" ? "$" : path}: ${reason}`);
  }
}

function checkNonEmptyString(errors: Errors, path: string, v: unknown): v is string {
...
function checkOptionalString(errors: Errors, path: string, v: unknown): void {
  if (v !== undefined && typeof v !== "string") {
    errors.at(path, `expected a string if present, got ${describe(v)}`);
  }
}

function checkFiniteNumber(errors: Errors, path: string, v: unknown): v is number {
...
function checkLat(errors: Errors, path: string, v: unknown): boolean {
  if (!checkFiniteNumber(errors, path, v)) return false;
  if (v < -90 || v > 90) {
...
function checkLng(errors: Errors, path: string, v: unknown): boolean {
  if (!checkFiniteNumber(errors, path, v)) return false;
  if (v < -180 || v > 180) {
...
function validateFootprint(errors: Errors, path: string, v: unknown): void {
  if (!isRecord(v)) {
  ...
  if (kind === "bbox") {
    ...
    if (latOk && (v["minLat"] as number) > (v["maxLat"] as number)) {
      errors.at(path, `bbox inverted: minLat ${String(v["minLat"])} > maxLat ${String(v["maxLat"])}`);
    }
    if (lngOk && (v["minLng"] as number) > (v["maxLng"] as number)) {
    ...
    errors.at(
      `${path}.kind`,
      `unknown footprint kind ${describe(kind)} (expected one of ${quoteList(FOOTPRINT_KINDS)})`
    );
  }
}
...
function validateSources(errors: Errors, path: string, v: unknown): Set<string> | null {
  if (!isRecord(v)) {
...
function validateProvenance(
  ...
  if (!Array.isArray(sources)) {
    ...
    for (let i = 0; i < sources.length; i++) {
      const id: unknown = sources[i];
      if (!checkNonEmptyString(errors, `${path}.sources[${i}]`, id)) continue;
      // Only cross-check ids when the pack's sources map itself parsed.
      if (sourceIds !== null && !sourceIds.has(id)) {
        errors.at(
          `${path}.sources[${i}]`,
          `cites unknown source id "${id}" (not in the pack's sources map)`
        );
      }
    }
  }
...
function validateHalfSlot(errors: Errors, path: string, v: unknown): void {
  if (!Array.isArray(v)) {
  ...
  for (let i = 0; i < v.length; i++) {
    const code: unknown = v[i];
    if (typeof code !== "string" || !(HALF_MONTH_CODES as readonly string[]).includes(code)) {
      errors.at(
        `${path}[${i}]`,
        `illegal code ${describe(code)} (expected one of ${quoteList(HALF_MONTH_CODES)})`
      );
      continue;
    }
    if (seen.has(code)) {
...
function validateGrid(errors: Errors, path: string, v: unknown): void {
  if (!isRecord(v)) {
...
function validateFrostRef(errors: Errors, path: string, v: unknown): void {
  if (v === undefined) return; // ref is optional (defaults pinned in DEFAULT_FROST_REF)
  if (!isRecord(v)) {
  ...
  if (
    threshold !== undefined &&
    !(FROST_THRESHOLDS_F as readonly number[]).includes(threshold as number)
  ) {
    errors.at(
      `${path}.thresholdF`,
      `illegal frost threshold ${describe(threshold)} (expected one of ${quoteList(FROST_THRESHOLDS_F)})`
    );
  }
  const probability = v["probabilityPct"];
  if (
    probability !== undefined &&
    !(FROST_PROBABILITIES_PCT as readonly number[]).includes(probability as number)
  ) {
    errors.at(
      `${path}.probabilityPct`,
      `illegal frost probability ${describe(probability)} (expected one of ${quoteList(FROST_PROBABILITIES_PCT)})`
    );
  }
}

function validateAnchor(errors: Errors, path: string, v: unknown): void {
  if (!isRecord(v)) {
  ...
  switch (kind) {
    ...
    case "soilTemp": {
      ...
      const threshold = v["thresholdF"];
      if (
        checkFiniteNumber(errors, `${path}.thresholdF`, threshold) &&
        (threshold < 32 || threshold > 110)
      ) {
        errors.at(
          `${path}.thresholdF`,
          `soil-temp threshold ${threshold}°F implausible (expected within [32, 110])`
        );
      }
    ...
    case "photoperiod": {
      ...
      if (direction !== "lengthening" && direction !== "shortening") {
        errors.at(
          `${path}.direction`,
          `expected "lengthening" or "shortening", got ${describe(direction)}`
        );
      }
      break;
    }
    ...
    default:
      errors.at(
        `${path}.kind`,
        `unknown anchor kind ${describe(kind)} (expected one of ${quoteList(ANCHOR_KINDS)})`
      );
  }
}

function validateOffsetDays(errors: Errors, path: string, v: unknown): void {
  if (!Array.isArray(v) || v.length !== 2) {
    errors.at(path, `expected a [earliest, latest] pair of day offsets, got ${describe(v)}`);
    return;
  }
  const lo: unknown = v[0];
  const hi: unknown = v[1];
  let pairOk = true;
  ...
  if (pairOk && (lo as number) > (hi as number)) {
    errors.at(path, `[${String(lo)}, ${String(hi)}] inverted (earliest must be ≤ latest)`);
  }
}

function validateEvents(errors: Errors, path: string, v: unknown): void {
  if (!Array.isArray(v)) {
  ...
  for (let i = 0; i < v.length; i++) {
    const ev: unknown = v[i];
    if (!isRecord(ev)) continue; // reported in second pass
    const id = ev["id"];
    if (typeof id === "string" && id !== "") {
      ...
      if (first !== undefined) {
        ...
        seenIds.set(id, i);
        if (ev["activity"] === "harvest") harvestIds.add(id);
      }
    }
  }
    ...
    if (activity === "harvest") {
      ...
      if (checkNonEmptyString(errors, `${p}.fromEventId`, from)) {
        if (!seenIds.has(from)) {
          ...
          errors.at(
            `${p}.fromEventId`,
            `references "${from}", which is itself a harvest (must reference a non-harvest planting event)`
          );
        }
      }
      const method = ev["method"];
      if (!(HARVEST_METHODS as readonly string[]).includes(method as string)) {
        errors.at(
          `${p}.method`,
          `expected one of ${quoteList(HARVEST_METHODS)}, got ${describe(method)}`
        );
      }
    } else if ((NON_HARVEST_ACTIVITIES as readonly string[]).includes(activity as string)) {
      // AnchoredEvent.
      validateAnchor(errors, `${p}.anchor`, ev["anchor"]);
      validateOffsetDays(errors, `${p}.offsetDays`, ev["offsetDays"]);
      if (ev["gate"] !== undefined) validateAnchor(errors, `${p}.gate`, ev["gate"]);
      ...
      errors.at(
        `${p}.activity`,
        `unknown activity ${describe(activity)} (expected "harvest" or one of ${quoteList(NON_HARVEST_ACTIVITIES)})`
      );
    }
  }
}
...
function validateTiming(errors: Errors, path: string, v: unknown): void {
  if (!isRecord(v)) {
...
function validateCropRow(
  ...
  if (checkNonEmptyString(errors, `${path}.crop`, crop)) {
    if (!SLUG_RE.test(crop)) {
      errors.at(
        `${path}.crop`,
        `"${crop}" is not slug-shaped (lowercase alphanumerics and single hyphens, e.g. "pea-snap-pole")`
      );
    } else {
...
export function validateRegionPack(
  value: unknown
): { ok: true; pack: RegionPack } | { ok: false; errors: string[] } {
  ...
  return { ok: true, pack: value as unknown as RegionPack };
}
